...
bool CMemory::Initialize()
{
	...
	{
		WRAP_IF_DEBUG(
			std::cout << "Failed to get Unturned PID.\n";
		)
	...
	while (!FindWindowA("UnityWndClass", NULL))
		Sleep(1000);
	...
	if (!Window)
	{
		...
		WRAP_IF_DEBUG(
			std::cout << "Failed to get Unturned window.\n";
		)
	...
	{
		WRAP_IF_DEBUG(
			std::cout << "Failed to get Unturned base address..\n";
		)
...
DWORD CMemory::GetProcessID()
{
	...
	do
	{
		if (wcscmp(pe32.szExeFile, L"Unturned.exe") == 0)
