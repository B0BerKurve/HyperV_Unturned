...
bool ImGui::Checkbox(const char* label, bool* v) {
    ...
    RenderFrame(frame_bb.Min, frame_bb.Max, ImColor(52.f, 195.f, 235.f, value->second * style.Alpha), 0, ROUNDING);
...
{
    return ColorEdit4(label, col, flags | ImGuiColorEditFlags_NoAlpha, nullptr);
}
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags, bool* rainbow)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    else if ((flags & ImGuiColorEditFlags_DisplayHex) != 0)
    {
        ...
        if (ImGui::Rainbow)
        {
            ////static bool rainbow_ = false;
            ImGui::Checkbox("enable rainbow", rainbow);
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions))
            OpenPopupOnItemClick("context");
    }
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if ((flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (flags & ImGuiColorEditFlags_DisplayRGB || (flags & ImGuiColorEditFlags__DisplayMask) == 0)
            if (ColorEdit4("##rgb", col, sub_flags | ImGuiColorEditFlags_DisplayRGB, nullptr))
            {
        ...
        if (flags & ImGuiColorEditFlags_DisplayHSV || (flags & ImGuiColorEditFlags__DisplayMask) == 0)
            value_changed |= ColorEdit4("##hsv", col, sub_flags | ImGuiColorEditFlags_DisplayHSV, nullptr);
        if (flags & ImGuiColorEditFlags_DisplayHex || (flags & ImGuiColorEditFlags__DisplayMask) == 0)
            value_changed |= ColorEdit4("##hex", col, sub_flags | ImGuiColorEditFlags_DisplayHex, nullptr);
        PopItemWidth();
    }
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (selected)
    {
        ...
        window->DrawList->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y+2), ImVec2(bb.Min.x + 1, bb.Max.y-2), menu.AccentColor.c());
        window->DrawList->AddRectFilled(ImVec2(bb.Min.x + 1, bb.Min.y+2), ImVec2(bb.Min.x + 2, bb.Max.y-2), menu.AccentColor.c());
        //RenderFrame(bb.Min, bb.Max, col, false, 0.0f);
        //RenderNavHighlight(bb, id, ImGuiNavHighlightFlags_TypeThin | ImGuiNavHighlightFlags_NoRounding);
    }
    else if (hovered)
    {
        color h_col = menu.AccentColor;
        h_col.a = 100 / 255.f;
